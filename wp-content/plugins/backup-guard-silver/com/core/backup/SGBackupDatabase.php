...
class SGBackupDatabase implements SGIMysqldumpDelegate
{
	...
	public function prepareQueryToExec($query)
	{
		...
		$currentArchiveVersion = SGConfig::get('SG_CURRENT_ARCHIVE_VERSION');

		if (SGBoot::isFeatureAvailable('BACKUP_WITH_MIGRATION')) {
			$query = SGReplacements::processReplacements($query, $this->restoreReplacements);
		}
		elseif ($currentArchiveVersion == 3) {
			//For now letting user to restore old archives with free version. In feature updates error will be thrown
			$query = str_replace(SG_DB_PREFIX_REPLACEMENT, SG_ENV_DB_PREFIX, $query);
		}
