...
define('SG_BACKUP_GUARD_VERSION', '1.1.24');
...
add_action( 'wp_ajax_nopriv_backup_guard_testReloadAvailabity', 'backup_guard_test_reload_availabity');

add_action( 'wp_ajax_backup_guard_manualBackup', 'backup_guard_manual_backup');
add_action( 'wp_ajax_nopriv_backup_guard_manualBackup', 'backup_guard_manual_backup_nopriv');
...
function backup_guard_test_reload_availabity()
{
	require_once(SG_PUBLIC_AJAX_PATH.'testReloadAvailabity.php');
}
...
function backup_guard_manual_backup_nopriv()
{
	$path = @$_GET['path'];
	$token = @$_GET['token'];

	if (backupGuardValidateApiCall($path, $token)) {
		require_once(SG_PUBLIC_AJAX_PATH.'manualBackup.php');
	}
}
